...
export async function GET(request: NextRequest) {
  try {
    ...
    const [usersRes, workRuleRes, attendancesRes, fieldValuesRes, salaryConfigsRes] = await Promise.all([
      ...
      db.select('work_rules', 'work_hours_per_day, work_days_per_month, overtime_rate_25, overtime_rate_50, health_insurance_rate, pension_rate, employment_ins_rate').limit(1).single(),
